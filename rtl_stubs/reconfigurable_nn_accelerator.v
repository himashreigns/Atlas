...
module reconfigurable_nn_accelerator #(
    ...
    task schedule_virtual_stages;
        ...
        begin
            ...
            // while+break replaced with a bounded for loop (Verilog-2001 /
            // Vivado synthesis does not support while or break).
            sched_active = 1;
            for (j = 0; j < MAX_LAYERS + MAX_VIRTUAL_STAGES; j = j + 1) begin
                if (sched_active && (layer_idx < total_layers)) begin
                    ...
                    if (allocated_macs[layer_idx] > 0) begin
                        ...
                        if (stage_idx >= MAX_VIRTUAL_STAGES) begin
                            ...
                            sched_active = 0;  // replaces break
                        end

                        // Deallocate previous stage to free resources
                        if (sched_active && (stage_idx > 0)) begin
    ...
    // Loop index shared across always blocks.
    // Verilog-2001 does not allow variable declarations in for-loop
    // initialisers or inside named begin..end blocks inside always.
    integer i;
    integer lidx;
        ...
        if (rst) begin
            ...
            case (exec_state)
                ...
                EXEC_COMPUTE: begin
                    ...
                    // Variable part-select [N-1:0] is not legal in Verilog-2001;
                    // AND-reduce the full vector instead (unused bits stay 0).
                    if (&layer_compute_done) begin
