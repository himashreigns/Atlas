...
class SearchMode(Enum):
    ...
    GRADIENT           = "gradient"
    SIMULATOR_GUIDED   = "simulator_guided"  # analytical pre-filter + sim rescoring
...
class BlockingOptimizer:
    ...
    def __init__(
        ...
        """analyzer: injectable cost model. Default is the analytical
        MemoryAccessAnalyzer; pass a SimulatorCostModel (from
        pysim.cost_model) for hardware-in-the-loop scoring.

        Both must implement .analyze(LoopNestConfig) -> Dict.
        The simulator analyzer additionally raises NotImplementedError if
        .traffic_continuous() is called, so SearchMode.GRADIENT is disabled
        when it is in use.
        """
        ...
        self._analyzer = analyzer or MemoryAccessAnalyzer(
    ...
    def search(
        ...
        elif mode == SearchMode.SIMULATOR_GUIDED:
            return self._simulator_guided_search(objective, verbose)
        else:
            raise ValueError(f"Unknown SearchMode: {mode}")

    # ------------------------------------------------------------------
    # SIMULATOR_GUIDED: analytical pre-filter on the heuristic candidate
    # set, then re-score the top-N survivors using whatever self._analyzer
    # is (intended to be a SimulatorCostModel).
    # ------------------------------------------------------------------
    def _simulator_guided_search(
        self, objective: str, verbose: bool, top_n: int = 20
    ) -> Tuple[LoopNestConfig, Dict[str, Dict]]:
        # Stage 1: cheap analytical pre-filter
        analytical = MemoryAccessAnalyzer(
            self.K, self.C, self.H, self.W, self.KH, self.KW,
            self.scratchpad_bytes,
        )
        configs = self._heuristic_configs()
        scored = []
        for blocking in configs:
            for order in TileLoopOrder:
                lnc = LoopNestConfig(blocking=blocking, tile_loop_order=order)
                metrics = analytical.analyze(lnc)
                scored.append((self._score(metrics, objective), lnc, metrics))
        scored.sort(key=lambda t: t[0])
        survivors = scored[:top_n]
        if verbose:
            print(f"[sim-guided] stage-1 produced {len(scored)} candidates; "
                  f"keeping top {len(survivors)} for simulator scoring.")

        # Stage 2: re-score with the (simulator) analyzer
        best_score = float("inf")
        best_lnc   = None
        all_stats  = {}
        for _, lnc, _ in survivors:
            metrics = self._analyzer.analyze(lnc)
            s = self._score(metrics, objective)
            key = f"{lnc}"
            all_stats[key] = metrics
            if s < best_score:
                best_score = s; best_lnc = lnc
        if best_lnc is None:
            best_lnc = survivors[0][1]
        return best_lnc, all_stats
