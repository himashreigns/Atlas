...
module tb_hdmi_out;
    ...
    always @(posedge pixel_clk) begin
        ...
        if (hdmi_de && !de_d) begin
            ...
            x_pos = 0;
            if (line_no <= 6)
                $display("[%0t] DE start line_no=%0d vcnt=%0d bank=%b buf0.w5.Cb=%02x buf1.w5.Cb=%02x",
                         $time, line_no, dut.vcnt, dut.vcnt[0],
                         dut.linebuf[10'd5][15:8], dut.linebuf[{1'b1,9'd5}][15:8]);
        end
        if (hdmi_de) begin
    ...
    // fetch trace
    always @(posedge aclk) begin
        if (dut.fstate == 2'd0 && dut.new_req && $time < 3000000)
            $display("[%0t] REQ line=%0d -> bank %b", $time, dut.line_req_num, dut.line_req_num[0]);
        if (dut.lb_we && dut.lb_waddr[8:0] == 9'd5 && $time < 3000000)
            $display("[%0t] WR bank=%b w5 data=%08x (f_line=%0d)", $time, dut.lb_waddr[9], dut.lb_wdata, dut.f_line);
    end
