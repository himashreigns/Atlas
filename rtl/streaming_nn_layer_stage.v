...
module streaming_nn_layer_stage #(
    ...
    parameter LAYER_TYPE      = "CONV",    // "CONV", "FC", "POOL"
    ...
    // Derived parameters
    parameter KERNEL_ROWS     = (LAYER_TYPE == "POOL") ? POOL_SIZE : KERNEL_SIZE,
    parameter OUT_ROWS        = (LAYER_TYPE == "CONV") ? ((IMG_ROWS + 2*PADDING - KERNEL_SIZE) / STRIDE + 1) :
                                (LAYER_TYPE == "POOL") ? (IMG_ROWS / POOL_SIZE) : 1,
    parameter OUT_COLS        = (LAYER_TYPE == "CONV") ? ((IMG_COLS + 2*PADDING - KERNEL_SIZE) / STRIDE + 1) :
                                (LAYER_TYPE == "POOL") ? (IMG_COLS / POOL_SIZE) : 1
)(
    input  wire clk,
    input  wire rst,
    ...
    localparam [1:0] MODE = (LAYER_TYPE == "CONV") ? 2'b00 :
                            (LAYER_TYPE == "FC")   ? 2'b01 :
                            (LAYER_TYPE == "POOL") ? 2'b10 : 2'b00;
    ...
    generate
        if (LAYER_TYPE == "CONV" || LAYER_TYPE == "POOL") begin : GEN_ROW_BUFFER
    ...
    nn_layer_control_unit #(
        ...
        .done(ctrl_done),
        .busy(ctrl_busy),
        .img_rows(IMG_ROWS[7:0]),
        .img_cols(IMG_COLS[7:0]),
        .kernel_size(KERNEL_SIZE[7:0]),
        .stride(STRIDE[7:0]),
        .padding(PADDING[7:0]),
        .in_channels(IN_CHANNELS[7:0]),
        .out_channels(OUT_CHANNELS[7:0]),
        .pool_size(POOL_SIZE[7:0]),
        .fc_input_size(FC_INPUT_SIZE[15:0]),
        .fc_output_size(FC_OUTPUT_SIZE[15:0]),
